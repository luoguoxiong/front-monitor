    ...
    function reportData(type = 1) {
      setTimeout(() => {
        ...
        if (ERRORLIST && ERRORLIST.length)
          conf.errorList = conf.errorList.concat(ERRORLIST);
        let w =
      ...
      resource.forEach((item) => {
        ...
        if (ajaxMsg) {
          ...
          json.decodedBodySize =
            json.decodedBodySize || ajaxMsg.decodedBodySize;
        }
            ...
            .then(function (res) {
              ...
              try {
                const responseURL = res.request.responseURL
                  ? res.request.responseURL.split("?")[0]
                  : "";
                const responseText = res.request.responseText;
                if (conf.ajaxMsg[responseURL])
                  conf.ajaxMsg[responseURL]["decodedBodySize"] =
                    responseText.length;
              } catch (e) {}
              return res;
            })
